...
### Updating the system and installing required packages
  1. The base operating system must be updated using the "apt" command. First, we must query the repositories for updates using the below command:
     ```
     sudo apt update
     ```
  ...
  1. Install the Argon ONE fan control drivers and scripts:
     ...
     argonone-config 
     ```
     By running the config script, you can customize the fan speed based on temperature. The available temperature ranges are in degrees Celsius: 55, 60, 65. I have set my configuration to 50, 75 and 100 percent respectfully. The is also an option to have the fan run always on.
